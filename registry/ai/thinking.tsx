 ...
 * Token      bg surface-2 (level: inset, no glow), radius sm, dot 8 brand, steps text-2 14px.
...
interface ThinkingProps extends React.DetailsHTMLAttributes<HTMLDetailsElement> {
  title: string
  live?: boolean
  steps: string[]
}
  ...
  return (
    <details data-level="inset" className={cn("group overflow-hidden rounded-sm bg-surface-2", className)} {...props}>
      <summary className="flex cursor-pointer list-none items-center gap-2 p-3 text-sm font-medium leading-5 text-text [&::-webkit-details-marker]:hidden">
        <span className={cn("size-2 shrink-0 rounded-full bg-brand", live && "animate-pulse")} aria-hidden />
